window.cnApp = {
    ...
    init: function () {
        ...
        $('#btnModalPrint').click(() => NexusPrint.printDocument('CreditNote', this._currentDocId));
        $('#btnModalPdf').click(() => NexusPrint.downloadDocument('CreditNote', this._currentDocId));
    },
                ...
                { data: 'grandTotal', className: 'text-end fw-bold text-success', render: d => parseFloat(d).toLocaleString(undefined, { minimumFractionDigits: 2 }) },
                {
                    ...
                    render: function (data, type, row) {
                        return `<button class="btn btn-sm btn-outline-dark shadow-sm me-1" onclick="cnApp.viewDocument(${row.id})" title="View Details"><i class="fa-solid fa-eye"></i></button>
                                <button class="btn btn-sm btn-outline-secondary shadow-sm me-1" onclick="NexusPrint.printDocument('CreditNote', ${row.id})" title="Print"><i class="fa-solid fa-print"></i></button>
                                <button class="btn btn-sm btn-outline-danger shadow-sm" onclick="NexusPrint.downloadDocument('CreditNote', ${row.id})" title="Download PDF"><i class="fa-solid fa-file-pdf"></i></button>`;
                    }
                }
            ],
            order: [[0, 'desc']],
